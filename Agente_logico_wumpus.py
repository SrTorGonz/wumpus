...
if getattr(sys, 'frozen', False):  # Si está ejecutándose como un ejecutable
    app_path = os.path.dirname(sys.executable)  # Obtiene la ruta del ejecutable
else:
    app_path = os.path.dirname(__file__)  # Obtiene la ruta del script
    
def resource_path(relative_path):
    """ Obtiene la ruta correcta de los recursos empaquetados en el .exe """
    try:
        # Si el script está congelado (en un .exe)
        if getattr(sys, 'frozen', False):
            # Usamos _MEIPASS para acceder al directorio temporal donde PyInstaller coloca los archivos
        ...
        return relative_path  # Devuelve la ruta relativa en caso de error
    
# Luego, usa resource_path para cargar las imágenes
agente_img = tk.PhotoImage(file=resource_path("agente.png")).subsample(5, 5)
wumpus_img = tk.PhotoImage(file=resource_path("wumpus.png")).subsample(5, 5)
hueco_img = tk.PhotoImage(file=resource_path("hueco.png")).subsample(10, 10)
oro_img = tk.PhotoImage(file=resource_path("oro.png")).subsample(5, 5)
hedor_img = tk.PhotoImage(file=resource_path("hedor.png")).subsample(5, 5)
brisa_img = tk.PhotoImage(file=resource_path("brisa.png")).subsample(5, 5)
